...
namespace InventorySystemCsharp
{
    public partial class Manager_home : Form
    {
        ...
        {
            if(model.Text!="" && comboBox1.Text!="" && price.Text!="" && instock.Text != "")
            {
                try
                {
                    ...
                    string query = "insert into `spareparts`(`model`,`part`,`type`,`price`,`instock`) values('" + model.Text.Trim() + "','"  + "','" + comboBox1.Text.Trim() + "','" + price.Text.Trim() + "','" + instock.Text.Trim() + "')";
        ...
        {
            if (u_modelTxt.Text != "" && u_typeCombo.Text != "" && u_priceTxt.Text != "" && u_stockTxt.Text != "")
            {
                try
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=inventorymgcsharp;");
                    string query = "update `spareparts` set `model`= '" + u_modelTxt.Text + "',`part`= '"  + "',`type`= '" + u_typeCombo.Text + "',`price`='" + u_priceTxt.Text + "', `instock`= '" + u_stockTxt.Text + "'where `id`= '"+ u_itemcodeTxt.Text +"' ";
                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        {
            if (d_modelTxt.Text != "" && d_typeCombo.Text != "" && d_priceTxt.Text != "" && d_instockTxt.Text != "")
            {
                try
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=inventorymgcsharp;");
